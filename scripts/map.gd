...
func generate_map():
	...
	var source_id = 0
	var atlas_coords = Vector2i(1, 5)
	while true:
		chunk = get_chunk(chunk)
		var position = world_to_tile(-1*(player.global_position))
		for x in range(position.x-size/2, position.x+size/2):
			for y in range(position.y-size/2, position.y+size/2):
